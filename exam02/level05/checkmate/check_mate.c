...
int	ft_rook(char **board, int y, int x)
{
	...
	j = y;
	while (--j >= 1 && ft_opiece(board[j][x]) != 1)
	{
		if (board[j][x] == 'K')
			return (1);
	}
...
int	ft_bishop(char **board, int y, int x)
{
	...
	i = y;
	j = x;
	while (++i <= len && ++j < len && ft_opiece(board[i][j]) != 1)
	{
		if (board[i][j] == 'K')
			return (1);
	}
	i = y;
	j = x;
	while (--i <= 1 && --j < 0 && ft_opiece(board[i][j]) != 1)
	{
		if (board[i][j] == 'K')
			return (1);
	}
	i = y;
	j = x;
	while (--i <= 1 && ++j < len && ft_opiece(board[i][j]) != 1)
	{
		if (board[i][j] == 'K')
			return (1);
	}
	...
	while (av[i])
	{
		...
		while (av[i][j])
		{
			...
			if (av[i][j] == 'B' && ft_bishop(av, i, j) == 1)
				return (1);
			if (av[i][j] == 'Q' && (ft_bishop(av, i, j) || ft_rook(av, i, j) == 1))
				return (1);
			j++;
		}
		i++;
